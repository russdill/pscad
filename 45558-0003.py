...
def part(m):
    ...
    row = pscad.row(pscad.donut(D("1.8") / 2, D("1.8") / 2 + D("0.5")), D("4.2"), 3)
    all = (
        ...
        pscad.hole(m.clearance, 0) + (
            pscad.up(D("7.30")) +
            pscad.row(pscad.circle(D(3) / 2), D("8.4"), 2)
        ),
        pscad.silk(m.silk) + (
            pscad.right(D("4.2")) +
            pscad.up(D("7.3") + D("6.6") - D("12.8") / 2) +
    ...
    silk = pscad.silk(m.silk) + (
        ...
        pscad.right(D("4.2") - 4 / 2) +
        pscad.up(D("7.3") + D("6.6")) +
        pscad.square((4, D("3.5")))
    )
